...
func (self *BaseController) auth() {
	...
	if len(arr) == 2 {
		...
		if userId > 0 {
			...
			utils.LOG.Debug("###### debug  ###### actionName  controllerName",self.actionName,self.controllerName)

			isHasAuth := strings.Contains(self.allowUrl, self.controllerName+"/"+self.actionName)
			noAuth := "ajaxsave/ajaxdel/table/loginin/loginout/getnodes/start/show/ajaxapisave/logs"
			isNoAuth := strings.Contains(noAuth, self.actionName)

			utils.LOG.Debug("###### debug  ###### isHasAuth  noAuth  isNoAuth  actionName",isHasAuth,noAuth,isNoAuth,self.actionName)
